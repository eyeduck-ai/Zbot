...
| 💾 **快取重試** | GSheet 寫入失敗時自動暫存資料，可一鍵重新上傳 |
...
1. 點擊上方按鈕下載 `Zbot.exe`
2. 執行 `Zbot.exe`，會顯示「Zbot 啟動中」進度視窗
3. 程式自動下載最新版 `Zbot_Server` 並啟動
4. 完成後自動開啟瀏覽器，並最小化至系統匣
5. 首次使用需設定 Supabase 連線資訊
6. 使用 EIP 帳號登入即可開始使用

### 修改設定

右鍵點擊系統匣圖示 → **開啟設定頁** → 可修改 Supabase 連線、開發模式等選項。

### 更新

程式會自動檢查更新。執行 `Zbot.exe` 時若有新版本會自動下載安裝（含進度條）。
...
│   ├── cache/               # 快取目錄 (GSheet 寫入失敗時暫存)
│   ├── app/
│   │   ├── routers/         # API 端點
│   │   ├── tasks/           # 組合型任務 (class-based)
│   │   ├── core/            # JobManager, TaskRegistry, CacheManager
│   │   └── db/              # Supabase 連接
